def DerivativeSignal():
    ...
    FirstDrev=[]
    SecondDrev=[]
    lengh=len(InputSignal)
    for j in range(1,lengh):
        FirstDrev.append(InputSignal[j]-InputSignal[j-1])
    for k in range(1,lengh-1):
        SecondDrev.append(InputSignal[k+1] - 2 * InputSignal[k] + InputSignal[k-1])
    print(FirstDrev,"_*_",SecondDrev)
    print(len(FirstDrev),"_*_",len(SecondDrev))
    """
    End
    """
